...
class UserA11yProfileService(
    ...
    fun getMyProfiles(userId: UUID): List<UserA11yProfileResponse> =
        profileRepository
            .findAllByUserUserIdOrderByCreatedAtAsc(userId)
            .map { it.toA11yProfileResponse() }
            .toList()
    ...
    fun createProfile(userId: UUID, dto: UserA11yProfileReq): UserA11yProfileResponse {
        // 프로필 이름 중복 체크
        if (profileRepository.existsByUserUserIdAndProfileInfoProfileName(userId, dto.profileName)) {
    ...
    fun updateProfile(userId: UUID, profileId: UUID, dto: UserA11yProfileReq) {
        val profile: UserA11yProfile = profileRepository.findByProfileIdAndUserUserId(profileId, userId)
            ?: throw DataNotFoundException("해당 접근성 프로필을 찾을 수 없습니다.")

        // 프로필 이름 중복 체크
        if (profile.profileInfo.profileName != dto.profileName &&
            profileRepository.existsByUserUserIdAndProfileInfoProfileName(userId, dto.profileName)
        ) {
            throw InvalidRequestException("이미 존재하는 프로필 이름입니다.")
        }
    ...
    fun deleteProfile(userId: UUID, profileId: UUID) {
        val deleted = profileRepository.deleteByProfileIdAndUserUserId(
            profileId = profileId,
            userId = userId
        )
